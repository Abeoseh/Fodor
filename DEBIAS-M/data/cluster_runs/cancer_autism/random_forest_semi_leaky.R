...
png(paste("./output/autism/autism_post_DEBIAS-M_RF_lognorm_ROC_", ID_label, ".png", sep=""))#, height = 24, width = 24)
...
phen <- phen[1,2]
title(paste("Training without: ", ID_label, " (", phen, ")",sep=""), line = + 2.5)
...
png(paste("./output/autism/autism_post_DEBIAS-M_RF_lognorm_histogram_", ID_label, ".png", sep=""))
...
g <- ggplot() + geom_histogram(data = filter(auc.df, Permutation == TRUE), aes(x = AUC), bins = 40) +
  ...
  labs(title = paste("Training without: ", ID_label, " (", phen, ")", sep=""), y = "count") +
...
if(file.exists("./csv_files/AUCs/semi_leaky_autism/autism_semi_leaky_AUCs.csv")){
  all <- read.csv("./csv_files/AUCs/semi_leaky_autism/autism_semi_leaky_AUCs.csv")
  all <- rbind(all, auc.df)
  write.csv(all, "./csv_files/AUCs/semi_leaky_autism/autism_semi_leaky_AUCs.csv", row.names = FALSE)
}else{(write.csv(auc.df, "./csv_files/AUCs/semi_leaky_autism/autism_semi_leaky_AUCs.csv", row.names = FALSE))}

write.csv(auc.df, paste("./csv_files/AUCs/semi_leaky_autism/autism_post_DEBIAS_",ID_label,"_AUCs.csv",sep=""), row.names = FALSE)
