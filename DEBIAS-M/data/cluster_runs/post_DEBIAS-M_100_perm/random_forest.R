...
g <- ggplot() + geom_histogram(data = filter(auc.df, Permutation == TRUE), aes(x = AUC), bins = 10) +
  ...
  annotate("label", x=min(auc.df$AUC)+.001, y=0.1, size = 3, label = paste("p= ", round(for.pval,3), sep="")) +
...
write.csv(auc.df, paste("./csv_files/",IDs[i],"_AUCs.csv",sep=""), row.names = FALSE)
